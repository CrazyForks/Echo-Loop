...
> 最后更新：2026-08-28（完成 ASR 模型归档安装、下载门控与弹窗交互迁移；TTS 安装机制保持不变）
...
- 冷启动已将重型初始化移出首帧路径，并以 Riverpod `AsyncValue` 统一本地数据 gate、失败重试与第三方初始化依赖；仍需在低端真机用启动日志核验首帧与本地数据 gate 的实际耗时。
