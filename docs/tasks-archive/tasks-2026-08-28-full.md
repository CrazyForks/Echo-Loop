...
- [x] 让 Kokoro/Piper TTS 模型复用通用归档安装器，实现 HTTP Range 断点续传、整包校验、staging 原子安装，以及用户取消/删除后的归档、部分文件、元数据和 staging 清理；补充 TTS 续传、原子安装失败保护与清理回归测试。**完成时间**: 2026-08-28 18:22
- [x] 统一 ASR/TTS 下载临时归档命名为 `_download_<resourceId>.<ext>`，保留旧 `_dl_` 备份排除兼容并更新 TTS 续传清理测试。**完成时间**: 2026-08-28 18:25

- [x] 修复 ASR 崩溃诊断误报：推理期间使用带时间戳的 pending 文件，正常结束后删除，残留文件启动时转换为 `asr-crash-<datetime>.log`。**完成时间**: 2026-08-28

- [x] 统一 ASR/TTS 模型下载失败态文案：移除失败说明中重复的“前往设置”提示，保留独立设置引导，并补充弹窗回归测试。**完成时间**: 2026-08-28 15:45

- [x] 修复 ASR 下载完成进入安装阶段时进度从 100% 回退到 95%，安装阶段保持显示 100%。**完成时间**: 2026-08-28

- [x] ASR 模型归档支持 HTTP Range 断点续传：网络中断保留 `.part` 与元数据，用户主动取消后清理该模型的归档、部分文件、元数据与 staging，并补充续传及取消清理回归测试。**完成时间**: 2026-08-28 15:25

- [x] 将 ASR 崩溃 marker 移至 `logs` 目录，并保留文件不再自动删除。**完成时间**: 2026-08-28

- [x] 修复 ASR 归档安装竞态：等待 ZIP 完整解压后再校验，下载阶段不创建 staging；模型本体与共享 VAD 分开安装，避免 VAD 缺失时重复下载模型，并补充安装器生命周期回归测试。**完成时间**: 2026-08-28 14:46

- [x] 修复 ASR 归档解包对固定顶层目录名的依赖：兼容任意版本目录名，将单一顶层目录内容统一展开安装，并补充 v1/v2 归档结构回归测试。**完成时间**: 2026-08-28

- [x] 修复 ASR 归档安装器的 staging 残留：仅 ASR 使用唯一临时目录，安装成功或失败均清理归档、`.part`、元数据和 staging 文件，并兼容清理旧固定 staging 目录；TTS 下载安装机制保持不变。**完成时间**: 2026-08-28 15:04

- [x] 将 ASR 模型 CDN 归档切换为带 `-v1` 版本后缀的资源，更新实测下载大小与 SHA-256，避免旧 CDN 缓存命中。**完成时间**: 2026-08-28

- [x] 将 ASR 模型迁移为独立的归档下载、整包 SHA 校验、staging 原子安装、`install.json` 状态、安装 Gate 和下载弹窗机制；接入 ASR 旧缓存迁移、VAD 共享资源与下载后不自动恢复录音/转录，统一确认/下载中/失败/取消交互与对应文案，并补充归档安装和状态回归测试。**完成时间**: 2026-08-28 15:04
...
- [x] 将发现精选合集播客入口图标改为本地 `apple-podcasts.svg`，移除固定网络图片请求。**完成时间**: 2026-08-28

- [x] 将发现精选合集中的播客入口文案更新为“Apple 播客 / Apple Podcasts”。**完成时间**: 2026-08-28

- [x] 将官方 catalog 普通刷新节流从 2 小时延长至 1 天；进入发现资源列表页和官方合集详情页时强制后台刷新，优先展示本地缓存并在新结果返回后更新页面。**完成时间**: 2026-08-27

- [x] 修复认证初始化期间订阅控制器误按匿名身份对账：Supabase 首次 `initialSession` 到达前保持 pending，登录用户首次权益请求直接使用真实身份，避免短暂显示 free。**完成时间**: 2026-08-27 23:25

- [x] 统一 Catalog 与 Paddle Plans 的缓存根目录、磁盘缓存、刷新节流与并发刷新机制；新增 PaddlePlansService、结果语义及旧 catalog 路径迁移；修复非持久化模式写盘及 unchanged meta 写入失败语义。**完成时间**: 2026-08-27 23:16

- [x] Google Play / RevenueCat 获取套餐失败或返回空套餐时，仍显示“商店支付遇到问题？使用网页支付”兜底入口，并补充 Paywall 回归测试。**完成时间**: 2026-08-27

- [x] 移除订阅套餐 Provider 启动预热的强制刷新，统一使用 1 天节流；会员页主动刷新仍可强制回源。**完成时间**: 2026-08-27

- [x] 统一官方 catalog 启动同步与其它入口遵守 2 小时刷新节流，移除启动后的强制刷新。**完成时间**: 2026-08-27

- [x] 将 Paddle API client 连接超时调整为 15 秒、响应超时调整为 30 秒。**完成时间**: 2026-08-27

- [x] 将订阅套餐普通刷新节流从 5 分钟调整为 1 天；进入会员订阅页仍保留强制刷新。**完成时间**: 2026-08-27

- [x] 为 `/version.json` 接入统一刷新节流机制，后台检查 1 小时内不重复请求，手动检查可强制刷新。**完成时间**: 2026-08-27
...
- [x] 统一离线发音包与离线词典的资源下载/安装机制：共用归档安装器、统一安装清单和独立后台初始化，迁移 `pronunciation/v2` 到固定 `pronunciation` 目录，并移除设置页手动下载项。**完成时间**: 2026-08-27
...
- [x] 修复复习统计中“现在可复习”与“今天到期”数量口径不一致造成的疑惑：明确展示逾期数量和今天到期数量。**完成时间**: 2026-08-27

- [x] 修复点击未下载 Piper 音色时未停止当前正在播放的试听。**完成时间**: 2026-08-27

- [x] 修复 TTS 预热生命周期：进入页面/弹窗、返回页面或切回主 tab 时重新预热，离开时停止；通过 root Navigator 路由观察处理全屏覆盖，并在模型 ready 后自动补发预热任务。**完成时间**: 2026-08-27

- [x] 统一 TTS 引擎加载链路：配置切换不绕过模型 ready 门控，模型下载完成后自动加载当前选中引擎；清理初始化失败的 in-flight Future，配置切换取消旧后台预热，并补充失败恢复、切换延迟加载与并发回归测试。**完成时间**: 2026-08-27

- [x] 修复普通 TTS 发音门控自动下载：未下载模型时先展示确认弹窗，用户点击“下载”后才开始下载；弹窗显示预计网络流量，下载完成不自动继续本次发音。**完成时间**: 2026-08-27
- [x] 优化 TTS 模型下载弹窗：明确区分需要下载、下载中和失败状态，展示预计流量与进度，支持取消下载并清理临时文件。**完成时间**: 2026-08-27
- [x] 修复 TTS 模型断网下载长时间卡在下载中：为 Kokoro/Piper 默认下载客户端增加连接、发送和接收超时，复用现有网络重试与失败状态。**完成时间**: 2026-08-27

- [x] 在尚未发布的 App Update Migration v6 中清理已废弃的 Kokoro/Piper TTS 模型下载 SP 标记，保留 ASR 下载与选中模型配置。**完成时间**: 2026-08-27

- [x] 统一 TTS 模型安装状态检查：ProviderScope 内首次加载并缓存安装状态，并发复用扫描 Future；删除等状态变化通过失效机制触发重新扫描，旧扫描结果使用 generation 防止回写。**完成时间**: 2026-08-27

- [x] 按需启动 TTS 引擎预热：查词面板、收藏页和设置页进入时触发后台加载；主动发音、试听及文本/音色预热统一等待对应引擎就绪，模型未下载时后台静默跳过且试听不弹窗。**完成时间**: 2026-08-27

- [x] 修复 TTS 引擎、模型变体与音色切换期间的加载/合成竞态：配置目标按代际隔离，旧引擎不回写、不播放，并在任务结束后安全释放；补充切换期间加载与合成的并发回归测试及带具体变体/音色的关键日志。**完成时间**: 2026-08-27
...
- [x] 为 AudioService 初始化增加前台播放降级：先提供可用的音频 handler/router，后台系统媒体会话失败时不阻断音频与视频播放，并在后续播放中静默单飞重试；补充失败、恢复与并发初始化回归测试。**完成时间**: 2026-08-25

- [x] 修复 Apple 账号页邮箱不可用时的展示：显示本地化“邮箱暂不可用”文案，不显示空邮箱或内部用户 ID；补充空白邮箱回归测试。**完成时间**: 2026-08-25

- [x] 修复 macOS Apple AI TTS 的 CAF 播放失败：原生合成改输出 WAV，提升 TTS 缓存命名空间避开旧 CAF，并补充平台 TTS 回归测试。**完成时间**: 2026-08-25

- [x] 澄清 `AudioService` 相关注释：明确其职责是连接系统媒体会话与播放器状态，覆盖锁屏、通知栏和蓝牙控制，不将其误解为仅负责音频解码播放。**完成时间**: 2026-08-25

- [x] 修复离线 ASR 默认模型回退：新用户及无效模型选择统一默认使用 Whisper Base Balanced，不再因可用模型列表顺序回退到 Fast。**完成时间**: 2026-08-25

- [x] 视频导入支持 MKV：加入统一视频扩展名白名单，并补充主素材分类、字幕配对和媒体类型回归测试。**完成时间**: 2026-08-25

- [x] 为离线词典本地检查、打开、首次下载与后台更新接入统一 StartupTrace，记录各步骤处理时长；下载成功与失败日志继续打印 `elapsedMs`，并补齐下载管理器回归断言。**完成时间**: 2026-08-25

- [x] 移除 ASR、Kokoro 与 Piper 的启动期模型状态恢复：Provider 改由已加载的 SharedPreferences 完成标记同步构造初始下载状态，不再在业务页放行后扫描模型目录或统计体积；已下载模型初始体积显示为未知，并补齐三类 Provider 回归测试。**完成时间**: 2026-08-25

- [x] 将 media_kit 初始化移出 `runApp()` 前关键路径：业务页首帧后后台预热，并由全局幂等初始化器保护视频与短音频 Player 创建；初始化失败仅记录技术日志、通过可复用应用级通知总线提示“播放失败”，关闭后下次播放自动重试。**完成时间**: 2026-08-24

- [x] 统一原生与 Flutter 冷启动视觉：Android/iOS 与本地数据 Gate 共用居中鹦鹉 Logo 和亮暗主题背景；移除 Gate 全屏转圈，保留本地数据安全 gate、失败重试与首页后台预热。**完成时间**: 2026-08-24 20:09
...
- [x] 冷启动首帧优先重构：`runApp` 后显式等待首帧，再串行执行目录/Drift 迁移、内置示例、音频、Firebase、Supabase、RevenueCat 和分析初始化；主导航在本地数据就绪 gate 前不创建业务页面，SDK 依赖预热等待后台服务完成，补齐关键结构化时序日志与 gate 单元测试。**完成时间**: 2026-08-24

- [x] 将 ASR、Kokoro 与 Piper 的本地模型文件校验移至学习 Tab 首帧后后台串行恢复；保留既有状态与交互逻辑，补齐 provider 回填回归测试和启动时序日志。**完成时间**: 2026-08-24

- [x] 补齐冷启动结构化时序日志：覆盖 Dart 入口、关键初始化步骤、`runApp`、Flutter 首帧、主导航壳和学习 Tab；日志落盘前缓冲、失败保留异常语义，并补齐追踪器单元测试。**完成时间**: 2026-08-24

- [x] 将旧收藏 FSRS 调度补齐从每次启动扫描改为 Drift v50→v51 一次性数据库迁移；保留有效快照历史，归档无效来源，并补齐升级后不重复执行的迁移回归测试。**完成时间**: 2026-08-24

- [x] 资源库标题连续点击 5 次后显示临时“全部 / All”音频入口，点击后恢复原音频列表访问。**完成时间**: 2026-08-24

- [x] 收窄收藏来源句右侧分段操作区至 42dp，并移除默认背景色，仅保留按下反馈。**完成时间**: 2026-08-24

- [x] 将收藏 Tab 来源句右侧圆形箭头改为铺满 item 高度的分段操作区，整块区域均可进入句子讲解页，并补充边缘点击回归测试。**完成时间**: 2026-08-24

- [x] 修复收藏词汇/意群来源句左侧缩进：无编号来源句与释义和分隔线共用左边界，收藏句编号布局不变，并补齐几何对齐回归测试。**完成时间**: 2026-08-24

- [x] 复用收藏句与收藏词汇/意群来源句 item：正文试听、右侧箭头进入句子讲解页；历史缺失定位时从字幕恢复，来源材料删除后显示不可点击的“来源材料未找到”提示，并补齐跳转、恢复和删除状态回归测试。**完成时间**: 2026-08-24

- [x] 收紧收藏词汇 item 来源句上下空白间距至紧凑布局。**完成时间**: 2026-08-24

- [x] 收藏词汇 item 移除来源句播放/停止图标；来源句保留整行播放交互，来源音频改为右对齐并增加右箭头提示可点击，补充词汇来源区域 widget 回归测试。**完成时间**: 2026-08-23

- [x] 收藏句子列表移除冗余播放/停止图标，改为显示音频分组内连续编号；保留整行播放切换，释放句子文本展示空间，并补充编号与图标移除回归测试。**完成时间**: 2026-08-23

- [x] 修复句子讲解视图收藏意群时可空父音频 ID 导致的编译错误：无父音频时跳过收藏，并补齐有效音频来源的 widget 回归测试。**完成时间**: 2026-08-23

- [x] 将资源库 AppBar 标题改为“我的资源库 / My Library”，保留并隐藏原合集/音频选择组件，增加后续彻底删除 TODO 及标题/隐藏状态回归测试。**完成时间**: 2026-08-23
